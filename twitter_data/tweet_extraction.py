...
def write_to_pos(tweet, count):
    # Prepare path
    path = 'txt_sentoken/pos/' + "{}.txt".format(count)
...
def write_to_neg(tweet, count):
    # Prepare path
    path = 'txt_sentoken/neg/' + "{}.txt".format(count)
    f = open(path, "w", encoding='utf-8')
    f.write(tweet)
    f.close()
...
# # Add New Line Symbol to seperate Tweets
# write_txt = ''
# for tweet in all_tweets:
...
# f = open("twitter_data.txt", "w", encoding='utf-8')
# f.write(write_txt)
# f.close()